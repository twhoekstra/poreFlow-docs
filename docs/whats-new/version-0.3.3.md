...
## Changelog
- Added `bessel_filter` function to `src/poreflow/utils.py` for signal processing
- Updated `.gitlab-ci.yml` with better publishing tests and managed Python support
- Added dynamic versioning system that reads from git tags
- Force use of managed Python during builds for consistency
- Improved dependency caching for faster CI/CD pipelines
- Updated publish job to only publish source distributions. Wheel distributions should
